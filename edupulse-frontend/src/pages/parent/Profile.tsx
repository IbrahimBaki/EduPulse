...
async function fetchMyProfile(): Promise<ParentProfile> {
  const { data } = await api.get('/auth/me')
  return data.data.user as ParentProfile
}
        ...
        <form className={styles.card} onSubmit={handleSubmit} noValidate>
          ...
          <div className={styles.formBody}>
            ...
            <div className={styles.section}>
              ...
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel} htmlFor="pp-name">Full name <span aria-hidden>*</span></label>
                  <input id="pp-name" type="text" className={inp('name')} value={form.name}
                    onChange={e => { setForm(f => ({ ...f, name: e.target.value })); setErrors(er => ({ ...er, name: undefined })) }} />
                  {errors.name && <p className={styles.fieldErr}>{errors.name}</p>}
                </div>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel} htmlFor="pp-email">Email <span aria-hidden>*</span></label>
                  <input id="pp-email" type="email" className={inp('email')} value={form.email}
              ...
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel} htmlFor="pp-phone">Phone</label>
                  <input id="pp-phone" type="tel" className={styles.formInput} value={form.phone}
                    onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
                </div>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel} htmlFor="pp-nid">National ID</label>
                  <input id="pp-nid" type="text" className={styles.formInput} value={form.national_id}
            ...
            <div className={styles.section}>
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel} htmlFor="pp-pw">New password</label>
                <input
                  id="pp-pw" type="password" className={inp('password')} value={form.password}
            ...
            {profile?.children && profile.children.length > 0 && (
              <div className={styles.section}>
                <p className={styles.sectionTitle}>My children ({profile.children.length})</p>
                <ul className={styles.childList}>
                  {profile.children.map(child => {
                    return (
                      <li key={child.id} className={styles.childItem}>
                        <UserAvatar name={child.name} avatarUrl={child.avatar_url} className={styles.childAvatar} />
